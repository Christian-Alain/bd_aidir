<?php 
    ...
    require('admin/config.php'); // On inclut la connexion à la base de données
    ...
    if(!empty($_POST['email']) && !empty($_POST['passeword'])) // Si il existe les champs email, password et qu'il sont pas vident
    {
        ...
        $check = $acces->prepare('SELECT * FROM users WHERE email = ?');
        ...
        if($row > 0)
        {
            ...
            if(filter_var($email, FILTER_VALIDATE_EMAIL))
            {
                ...
                if(password_verify($password, $data['passeword']))
                {
